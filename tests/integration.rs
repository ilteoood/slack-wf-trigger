...
use wf_trigger::config::{self, Matcher, Rule};
use wf_trigger::cursors::CursorStore;
use wf_trigger::slack::{ChannelRef, SlackApi};
use wf_trigger::trigger;
...
fn write_cursors(dir: &Path, body: &str) {
    std::fs::write(dir.join(".wf-trigger.cursors.json"), body).unwrap();
}
...
async fn ac017_fresh_install_seeds_cursor_without_spawning_commands() {
    ...
    let dir = TempDir::new().unwrap();
    assert!(!dir.path().join(".wf-trigger.cursors.json").exists());
    let api = SlackApi::with_base("xoxp-test", server.uri()).unwrap();
